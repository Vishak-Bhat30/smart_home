"""
Finetuning Meta-Llama-3-8B-Instruct on Smart Home Commands (Multi-GPU DDP)
==========================================================================
Full-precision LoRA (no quantization) with DDP across 8 GPUs.
Each GPU fits the 8B model in bf16 (~16GB) easily on 183GB B200s.

Launch: accelerate launch --config_file accelerate_config.yaml finetune_llama.py
"""
...
GRAD_ACCUM_STEPS = 1         # effective batch = 16 * 8 GPUs = 128
LEARNING_RATE = 2e-4
MAX_SEQ_LENGTH = 512
WEIGHT_DECAY = 0.01
LR_SCHEDULER = "cosine"
...
def main():
    ...
    if local_rank == 0:
        print("=" * 60)
        print("Meta-Llama-3-8B-Instruct LoRA Finetuning (8-GPU DDP)")
    ...
    # 2. Model — full bf16, no quantization (183GB GPUs can easily fit 16GB model)
    ...
    # 3. LoRA config (applied by SFTTrainer automatically)
    ...
    # 6. Trainer — pass peft_config so SFTTrainer handles LoRA + DDP properly
    trainer = SFTTrainer(
        model=model,
    ...
    if local_rank == 0:
        ...
        plt.xlabel("Steps")
        plt.ylabel("Loss")
        plt.title("LLaMA-3-8B LoRA Finetuning — Training Loss")
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(results_dir, "llama_training_loss.png"), dpi=150)
        plt.close()
        print(f"  ✓ Training loss plot saved to results/llama_training_loss.png")
